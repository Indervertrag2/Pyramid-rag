  ...
  const handleCreateUser = async () => {
    try {
      ...
      const message = axios.isAxiosError(error)
        ? (error.response?.data as { detail?: string } | undefined)?.detail || 'Unbekannter Fehler'
        : 'Fehler beim Erstellen des Benutzers';
      alert(`Fehler beim Erstellen des Benutzers: ${message}`);
    }
  };

  const handleDeleteUser = async (userId: string) => {
    if (!confirm('Sicher, dass Sie diesen Benutzer lAschen mAchten?')) return;
  ...
  const handleFileUpload = async () => {
    ...
    try {
      for (const file of Array.from(selectedFiles)) {
        ...
        await apiClient.post('/api/v1/documents/upload-simple', formData, {
          headers: {
